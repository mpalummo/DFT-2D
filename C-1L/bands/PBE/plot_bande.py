...
with open('Bande_graphene_PBE.dat.gnu', 'r') as file:
...
for line in lines:
    if line.strip(): 
        ...
        colonna2.append(float(values[1]) + 1.6656 ) 
    else:
...
for i, sublist in enumerate(sublists):
    if i == 3:
        plt.plot(sublist[0], sublist[1], color='red',alpha=0.7, lw=3, label="$Valence \ Bands$")
    elif i == 4:
        plt.plot(sublist[0], sublist[1], color='blue',alpha=0.7, lw=3, label="$Conduction \ Bands$")
    else:
...
plt.title("$PBE-Bands-of-Graphene$")
