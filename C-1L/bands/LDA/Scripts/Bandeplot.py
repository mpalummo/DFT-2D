...
with open('Graphene_LDA.dat.gnu', 'r') as file:
...
for line in lines:
    if line.strip(): 
        ...
        colonna2.append(float(values[1]) + 1.8675 ) 
    else:
...
for i, sublist in enumerate(sublists):
    if i == 3:
        plt.plot(sublist[0], sublist[1], color='red',alpha=0.7, lw=3, label="$Banda\ di \ Valenza$")
    elif i == 4:
        plt.plot(sublist[0], sublist[1], color='blue',alpha=0.7, lw=3, label="$Banda \ di  \ Conduzione$")
    else:
...
plt.title("$LDA-Bands-of-Graphene$")
