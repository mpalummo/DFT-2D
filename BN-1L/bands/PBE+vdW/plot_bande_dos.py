...
for line in lines:
    if line.strip():  
        ...
        colonna2_bs.append(float(values[1]) +1.8)  
    else:
...
with open('graphene_PBE_vdW_DOS50.dat', 'r') as file:
    ...
    for riga in righe[1:]:
        ...
        if float(valori[0])+1.8 <= 0:
            colonnasno1.append((float(valori[0]))+1.8)
            colonnasno2.append((float(valori[1])))
        else:
            colonnaso1.append((float(valori[0]))+1.8)
...
axs[1].set_title("$D.O.S. \ of \ Graphene$")
